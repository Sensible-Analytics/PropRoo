...
def _execute_values_postgres(
    table_name: str, columns: List[str], records: List[Tuple], chunk_size: int = 5000
):
    if not records:
        return
    ...
    for i in range(0, len(records), chunk_size):
        ...
        psycopg2.extras.execute_values(cur, insert_query, chunk, page_size=1000)
        conn.commit()
        logger.info(
            f"Inserted {min(i + chunk_size, len(records))} / {len(records)} records into {table_name}"
        )
    cur.close()
    conn.close()
...
def calculate_growth_metrics() -> None:
    ...
    try:
            ...
            property_stats.append(
                {
                    ...
                    "property_post_code": int(first_sale["property_post_code"])
                    if pd.notnull(first_sale["property_post_code"])
                    else 0,
                }
            )
            ...
            db_property_records = [
                {
                    k: v
                    for k, v in p.items()
                    if k in PropertyGrowth.__table__.columns.keys()
                }
        ...
        for year in years_range:
            ...
            # Street Level
            street_stats = (
                year_active_df.groupby(
                    ["property_street_name", "property_locality", "property_post_code"]
                )
                .agg(avg_cagr=("cagr", "mean"), property_count=("property_id", "count"))
                .reset_index()
            )
            ...
            # Suburb Level
            suburb_stats = (
                year_active_df.groupby(["property_locality"])
                .agg(avg_cagr=("cagr", "mean"), property_count=("property_id", "count"))
                .reset_index()
            )
        ...
        if street_records:
            if IS_POSTGRES:
                ...
                _execute_values_postgres("street_growth", cols, recs)
            else:
        ...
        if suburb_records:
            if IS_POSTGRES:
                cols = ["suburb", "year", "avg_cagr", "property_count"]
                recs = [
                    (r["suburb"], r["year"], r["avg_cagr"], r["property_count"])
                    for r in suburb_records
                ]
                _execute_values_postgres("suburb_growth", cols, recs)
            else:
        ...
        street_cagr_overall = (
            ...
            .agg(avg_cagr=("cagr", "mean"))
            .reset_index()
        )

        street_summary_df = street_summary_stats.merge(
            street_cagr_overall,
            on=["property_street_name", "property_locality", "property_post_code"],
            how="left",
        )

        # Identify top performers (e.g., top 10% by CAGR)
        cagr_threshold = (
            street_summary_df["avg_cagr"].quantile(0.9)
            if not street_summary_df["avg_cagr"].dropna().empty
            else 0
        )
        street_summary_df["is_top_performer"] = (
            street_summary_df["avg_cagr"] >= cagr_threshold
        ).astype(int)
        ...
        if street_summary_records:
            if IS_POSTGRES:
                ...
                recs = [
                    ...
                    for r in street_summary_records
                ]
                _execute_values_postgres("street_summary", cols, recs)
            else:
                for i in range(0, len(street_summary_records), 10000):
                    db.bulk_insert_mappings(
                        StreetSummary, street_summary_records[i : i + 10000]
                    )

        # Suburb Summary
        suburb_summary_stats = (
        ...
        suburb_cagr_overall = (
            growth_df.groupby(["property_locality"])
            .agg(avg_cagr=("cagr", "mean"))
            .reset_index()
        )

        suburb_summary_df = suburb_summary_stats.merge(
            suburb_cagr_overall, on=["property_locality"], how="left"
        )

        cagr_threshold_suburb = (
            suburb_summary_df["avg_cagr"].quantile(0.9)
            if not suburb_summary_df["avg_cagr"].dropna().empty
            else 0
        )
        suburb_summary_df["is_top_performer"] = (
            suburb_summary_df["avg_cagr"] >= cagr_threshold_suburb
        ).astype(int)
        ...
        if suburb_summary_records:
            if IS_POSTGRES:
                ...
                recs = [
                    ...
                    for r in suburb_summary_records
                ]
                _execute_values_postgres("suburb_summary", cols, recs)
            else:
                db.bulk_insert_mappings(SuburbSummary, suburb_summary_records)

        logger.info(
            f"Summarized {len(street_summary_records)} streets and {len(suburb_summary_records)} suburbs."
        )
        db.commit()

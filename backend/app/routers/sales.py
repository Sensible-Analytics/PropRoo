...
def get_sales(
    ...
    if min_growth is not None:
        query = query.filter(
            PropertyGrowth.cagr >= (min_growth / 100.0)
...
def get_monthly_median(
    ...
    month_expr = (
        func.to_char(Sale.contract_date, "YYYY-MM")
        if IS_POSTGRES
        else func.strftime("%Y-%m", Sale.contract_date)
    )
    q = db.query(
    ...
    return [
        {"month": r.month, "avg_price": r.avg_price, "count": r.count}
        for r in results
...
def get_top_suburbs(
    ...
    results = (
        q.group_by(Sale.property_locality).order_by(desc("count")).limit(limit).all()
    )

    return [
        {"suburb": r.property_locality, "count": r.count, "avg_price": r.avg_price}
        for r in results
    ]
...
def get_property_history(
    property_id: str, background_tasks: BackgroundTasks, db: Session = Depends(get_db)
):
    ...
    def refresh_listings_task(pid):
        ...
        try:
            ...
            if latest_sale:
                ...
                if (
                    not latest_sale.listings_last_checked
                    or latest_sale.listings_last_checked
...
def get_suburb_cagr(
    suburb: str, year: Optional[int] = 2024, db: Session = Depends(get_db)
):
    """Get average CAGR and property count for a suburb for a given year"""
    result = (
        db.query(SuburbGrowth)
        .filter(SuburbGrowth.suburb == suburb, SuburbGrowth.year == year)
        .first()
    )
...
def get_street_trend(street_name: str, suburb: str, db: Session = Depends(get_db)):
    ...
    results = (
        db.query(StreetGrowth)
        .filter(StreetGrowth.street_name == street_name, StreetGrowth.suburb == suburb)
        .order_by(StreetGrowth.year.asc())
        .all()
    )
    return [
        {"year": r.year, "avg_cagr": r.avg_cagr, "property_count": r.property_count}
        for r in results
    ]
...
def get_suburb_trend(suburb: str, db: Session = Depends(get_db)):
    ...
    results = (
        db.query(SuburbGrowth)
        .filter(SuburbGrowth.suburb == suburb)
        .order_by(SuburbGrowth.year.asc())
        .all()
    )
    return [
        {"year": r.year, "avg_cagr": r.avg_cagr, "property_count": r.property_count}
...
def get_global_summary(db: Session = Depends(get_db)):
    ...
    top_suburbs = (
        db.query(SuburbSummary).order_by(desc(SuburbSummary.avg_cagr)).limit(5).all()
    )
    top_streets = (
        db.query(StreetSummary).order_by(desc(StreetSummary.avg_cagr)).limit(5).all()
    )
...
def get_top_performers(
    year: int = 2024, property_type: Optional[str] = None, db: Session = Depends(get_db)
):
    ...
    streets = (
        db.query(StreetGrowth)
        .filter(StreetGrowth.year == year)
        .order_by(desc(StreetGrowth.avg_cagr))
        .limit(10)
        .all()
    )
    ...
    suburb_act_q = db.query(
        Sale.property_locality, func.count(Sale.id).label("sales_count")
    ).filter(extract("year", Sale.contract_date) == year)
    ...
    suburb_activity = (
        suburb_act_q.group_by(Sale.property_locality)
        .order_by(desc("sales_count"))
        .limit(10)
        .all()
    )
    street_activity = (
        street_act_q.group_by(Sale.property_street_name, Sale.property_locality)
        .order_by(desc("sales_count"))
        .limit(10)
        .all()
    )
    ...
    return {
        ...
        "activity": {
            "suburbs": [
                {"suburb": s.property_locality, "sales_count": s.sales_count}
                for s in suburb_activity
            ],
            "streets": [
                {
                    "street_name": s.property_street_name,
                    "suburb": s.property_locality,
                    "sales_count": s.sales_count,
                }
                for s in street_activity
            ],
        },
    }
...
def get_neighboring_suburbs(suburb: str, db: Session = Depends(get_db)):
    ...
    target = (
        db.query(
            func.avg(Sale.latitude).label("lat"), func.avg(Sale.longitude).label("lon")
        )
    ...
    neighbors = (
        db.query(
            ...
            (
                (func.avg(Sale.latitude) - target.lat)
                * (func.avg(Sale.latitude) - target.lat)
                + (func.avg(Sale.longitude) - target.lon)
                * (func.avg(Sale.longitude) - target.lon)
            ).label("dist"),
        )
        .filter(Sale.property_locality != suburb, Sale.latitude != None)
    ...
    neighbor_map = {
        n.property_locality: {"lat": n.lat, "lon": n.lon} for n in neighbors
    }
    return [
        {
            "suburb": s.suburb,
...
def get_neighboring_streets(
    street_name: str, suburb: str, db: Session = Depends(get_db)
):
    ...
    target = (
        ...
        .filter(
            Sale.property_street_name == street_name,
            Sale.property_locality == suburb,
            Sale.latitude != None,
        )
        .first()
    )
    ...
    if not target.lat:
        ...
        streets = (
            ...
            .filter(
                StreetSummary.suburb == suburb, StreetSummary.street_name != street_name
            )
            .limit(10)
            .all()
        )
        ...
        neighbors = (
            ...
            .group_by(Sale.property_street_name, Sale.property_locality)
            .order_by("dist")
            .limit(20)
            .all()
        )
        ...
        for n in neighbors:
            s_stat = (
                db.query(StreetSummary)
                .filter(
                    StreetSummary.street_name == n.property_street_name,
                    StreetSummary.suburb == n.property_locality,
                )
                .first()
            )
            if s_stat:
    ...
    neighbors = (
        db.query(
            ...
            (
                ...
                + (Sale.longitude - target.longitude)
                * (Sale.longitude - target.longitude)
            ).label("dist"),
        )
    ...
    if level == "suburb":
        ...
        for i, tp in enumerate(top_performers):
            ...
            neighbor_stats = (
                ...
                .filter(SuburbSummary.suburb.in_(neighbor_names))
                .all()
            )
            neighbor_map = {
                n.property_locality: {"lat": n.lat, "lon": n.lon} for n in neighbors_raw
            }
        ...
        for i, tp in enumerate(top_performers):
            target = (
                ...
                .filter(
                    Sale.property_street_name == tp.street_name,
                    Sale.property_locality == tp.suburb,
                    Sale.latitude != None,
                )
                .first()
            )
